...
const VideoRoom: React.FC<VideoRoomProps> = ({
  ...
  // Add currentUserRole state to determine if user is mentor or mentee
  const [currentUserRole, setCurrentUserRole] = useState<'mentor' | 'mentee' | null>(null);
  ...
  useEffect(() => {
    ...
    if (user) {
      if (user.id === mentorId) {
        setCurrentUserRole('mentor');
      } else if (user.id === menteeId) {
        setCurrentUserRole('mentee');
      }
    }
  ...
  }, [bookingId, mentorId, menteeId, user]);
  ...
  const otherUser = currentUserRole === 'mentor' ? sessionDetails?.mentees : sessionDetails?.mentors;
